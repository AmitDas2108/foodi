...
const Catagories = () => {
  ...
  return (
    <div id="home" className={`max-w-screen-2xl container mx-auto xl:px-24 px-4 py-16 bg-gradient-to-b from-0% from-[#F5EDED] to-[#ECFFE6] to-100% ${isDarkMode ? 'dark' : ''}`}>
        ...
        {/* category cards */}
        <div className='flex flex-col sm:flex-row flex-wrap gap-8 justify-around items-center mt-12  drop-shadow-[0_0px_10px_rgba(0,0,0,0.25)]'>
            {
                categoryItems.map((item, i) => (
                    <div key={i} className='drop-shadow-[0_0px_10px_rgba(0,0,0,0.25)] rounded-md bg-white py-6 px-5 w-72 mx-auto text-center cursor-pointer hover:-translate-y-4 transition-all duration-300 z-10'>
